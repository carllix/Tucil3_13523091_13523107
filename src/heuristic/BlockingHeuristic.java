...
public class BlockingHeuristic implements Heuristic {
    ...
    public int calculate(Board board) { // kalau ga valid 
        ...
        if (exitRow < 0) dirRow = -1;         //atas
        else if (exitRow >= board.getRows()) dirRow = 1;  //bawah
        else if (exitCol < 0) dirCol = -1;    // kiri
        else if (exitCol >= board.getCols()) dirCol = 1;  // kanan
        else return Integer.MAX_VALUE;        // ga valid
        ...
        if (!primary.isHorizontal()) {
            if (dirRow > 0) startRow = row + length - 1; 
        } 
        else {
            if (dirCol > 0) startCol = col + length - 1; 
        }
